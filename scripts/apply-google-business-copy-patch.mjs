...
const catalogAnchor = "catalog=catalog.map(item=>{const {purchaseUrl,url,...rest}=item; const rawImage=text(rest.image).replace(/^\\.\\/(?=https?:\\/\\/)/,''); const image=rawImage.startsWith(site+'/')?'./'+rawImage.slice(site.length+1):rawImage; return {...rest,image};});";
const catalogReplacement = `${catalogAnchor}\n\nconst publishedState = new Map(catalog.map(item => [text(item.id).toLowerCase(), item]));\nconst unpublishedMissingGoogleBusinessPostIds = products\n  .filter(product => {\n    const id = text(product.productId || product.id).toLowerCase();\n    return id && !publishedState.has(id) && !String(product.googleBusinessPost || '').trim();\n  })\n  .map(product => String(product.id || product.productId || '(unknown)'));\nif (unpublishedMissingGoogleBusinessPostIds.length) {\n  throw new Error('googleBusinessPost is required for unpublished LPs: ' + unpublishedMissingGoogleBusinessPostIds.join(', '));\n}\nconst productsToPublish = products.filter(product => {\n  const id = text(product.productId || product.id).toLowerCase();\n  if (!id) return true;\n  const existing = publishedState.get(id);\n  const hasGoogleBusinessPost = Boolean(String(product.googleBusinessPost || '').trim());\n  if (!existing) return true;\n  if (!hasGoogleBusinessPost) return false;\n  return true;\n});\nawait fs.writeFile('/tmp/published_products_source.json', JSON.stringify(productsToPublish, null, 2) + '\\n');\nconsole.log('LP publish candidates: ' + productsToPublish.length + ' / feed products: ' + products.length);\nconsole.log('LP publish candidate IDs: ' + productsToPublish.map(product => text(product.productId || product.id).toLowerCase()).filter(Boolean).join(', '));`;
if (!publisher.includes(catalogAnchor)) throw new Error('google-business-copy patch: catalog state anchor not found');
publisher = publisher.replace(catalogAnchor, catalogReplacement);

const loopAnchor = 'for (const product of products) {';
if (!publisher.includes(loopAnchor)) throw new Error('google-business-copy patch: publish loop anchor not found');
publisher = publisher.replace(loopAnchor, 'for (const product of productsToPublish) {');
