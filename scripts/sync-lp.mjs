...
for (const raw of products) {
  if (raw.lpPublished === false) continue;
  ...
  for (const [role, url] of imageEntries) {
    ...
    if (!imageResponse.ok) throw new Error(`Image download failed for ${id}/${role}: ${imageResponse.status}`);
    const type = imageResponse.headers.get("content-type") || "image/jpeg";
    const ext = type.includes("webp") ? "webp" : type.includes("png") ? "png" : "jpg";
    const file = `${role}.${ext}`;
    await fs.writeFile(path.join(imageDir, file), Buffer.from(await imageResponse.arrayBuffer()));
...
<section class="closing"><h2>${esc(raw.closingCopy || raw.catchCopy || raw.productName)}</h2><a class="cta" href="${esc(buy.toString())}&utm_content=bottom_cta">公式オンラインショップへ</a></section>
  ...
  list.push({ id, name: raw.productName, category: raw.category || "", description: raw.cardDescription || raw.description || "", image: `./assets/products/${id}/${productImage.file}`, updatedAt: raw.updatedAt || payload.updatedAt || new Date().toISOString() });
}

if (!list.length) throw new Error("Feed contained no publishable products.");
